...
{
	const float r0 = Utils::SampleBlueNoiseMap(xxPos, yyPos, 0), r1 = Utils::SampleBlueNoiseMap(xxPos, yyPos, 0);
...
float3 Game::SampleNEEShaded( Ray& ray )
{
	...
	for (int depth = 0; depth < m_maxDepth; depth++)
	{
		...
		float r0 = Utils::SampleBlueNoiseMap(xxPos, yyPos, 0);
...
void Game::Tick( float deltaTime )
{
	if (samplesTaken == 0)
	{
		cout << Utils::CalculateRMSEValueFromFile("BlueNoise.txt");
		Utils::LoadImageA();
	}
	...
	if (samplesTaken == 1025)
	{
		Utils::SaveToFile("BlueNoise.txt", frameSample);
	}
}