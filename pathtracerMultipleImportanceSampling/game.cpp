			...
			if (m_NEE)
			{
				...
				if (NdotL > 0)
				{
					...
					if (!scene.IsOccluded(r))
					{
						...
						E += T * (NdotL / misPDF) * sampledBRDF * scene.lightColor;
					}
				}
			}
			else
...
void Game::Tick( float deltaTime )
{
	...
	float rmseValue = Utils::rmsValue(values, Utils::GetGroundTruth());
	rmseValues.push_back(Utils::RoundFloat(rmseValue));

	if (samplesTaken == 1025)
	{
		Utils::SaveToFile("RMSEValuesMultipleImportanceSampling.txt", rmseValues);
	}
}
